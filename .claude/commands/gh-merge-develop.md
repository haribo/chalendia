...
- Run `just dev-tmp-clean` — removes `tmp/branch/<branch>` (its branch was just deleted) and garbage-collects artifacts from any other vanished branch (screenshots and one-off scripts captured for UI-validation gate — see `CLAUDE.md` § Frontend UI changes)
