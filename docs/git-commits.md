...
Recommended on all commits. Matches the area of change (domain, layer, tool).

Examples: `catalog`, `cart`, `orders`, `api`, `design`, `justfile`, `github`
...
```
feat(api)!: remove the legacy cart endpoint
```
